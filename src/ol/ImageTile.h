...
public:
    ...
    */
    std::vector<ol::events::EventsKey> imageListenerKeys_ ;

public:
    //const ImageTile = function(tileCoord, state, src, crossOrigin, tileLoadFunction, opt_options) {
    ImageTile (ol::tilecoord::TileCoord const &tileCoord, ol::TileState::_t state, 
        std::string const &src = std::string(),
        std::string const &crossOrigin = std::string(),
        void *tileLoadFunction = 0, 
        ol::Tile::Options const &opt_options = ol::Tile::Options())
        : Tile(tileCoord, state, opt_options)
