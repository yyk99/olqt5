...
template<typename T>
int revNumCompare (T const &a, T const &b) 
{
    return int(b - a);
}

TEST(ol_array, binarySearch)
{
    ...
    {
        int pos = binarySearch(a, "100");
        EXPECT_TRUE(pos < 0); // .to.be.ok();
    }
    //      });
    //      it('should have an insertion point of 0 for \'100\'', function() {
    {
        int pos = binarySearch(a, "100");
        EXPECT_EQ(0, insertionPoint(pos));// .to.be(0);
    }
    //      });
    //      it('should not find \'zzz0\'', function() {
    {
        int pos = binarySearch(a, "zzz0");
        EXPECT_TRUE(pos < 0)/*.to.be.ok()*/;
    }
    //      });
    ...
    {
        int pos = binarySearch(a, "zzz0");
        EXPECT_EQ(24, insertionPoint(pos)); // .to.be(a.length);
    }
    //        }
    //      );
    ...
    //        function() {
    {
        int pos = binarySearch(a, "BA");
        EXPECT_EQ(10, insertionPoint(pos)); // .to.be(10);
    }
    //      );
    //    });
}
...
TEST(ol_array, default_comparison)
{
    ...
    //      function() {
    {
        ...
        EXPECT_EQ(0, binarySearch(c, "only item"));
        //        });
        //        it('should not find \'a\'', function() {
        //          expect(binarySearch(c, 'a') < 0).to.be.ok();
        EXPECT_LT(binarySearch(c, "a"), 0);
        //        });
        //        it('should have an insertion point of 0 for \'a\'',
        //          function() {
        {
            int pos = binarySearch(c, "a");
            EXPECT_EQ(0, insertionPoint(pos));// .to.be(0);
        }
        //          }
        //        );
        ...
        EXPECT_TRUE(binarySearch(c, "z") < 0);// .to.be.ok();
    ...
    // describe('default comparison on array of Number(s)', function() {
    {
        ...
        {
            int pos = binarySearch(d, 54255);
            EXPECT_EQ(16, insertionPoint(pos));
        }
        //      );
        //      it('should not find 1.1', function() {
        {
            int pos = binarySearch(d, 1.1);
            EXPECT_TRUE(pos < 0);
        }
        //      });
...
TEST(ol_array, custom_comparison)
{
    {
        std::vector<double> e = {
        ...
        {
            int pos = binarySearch(e, 54254.0, revNumCompare<double>);
            EXPECT_EQ(0, pos);
        }
        //        });
        //        it('should find -897123.9 at index ' + (e.length - 1), function() {
        {
            int pos = binarySearch(e, -897123.9, revNumCompare<double>);
            EXPECT_EQ(15, pos); // .to.be(e.length - 1);
        }
        //        });
...
TEST(ol_array, isSorted)
{
    //  describe('isSorted', function() {
    //    it('works with just an array as argument', function() {
    {
        //      expect(isSorted([1, 2, 3])).to.be(true);
        EXPECT_TRUE(isSorted(std::vector<ol::number_t>({ 1, 2, 3 })));
        //      expect(isSorted([1, 2, 2])).to.be(true);
        EXPECT_TRUE(isSorted(std::vector<ol::number_t>({ 1, 2, 2 })));
        //      expect(isSorted([1, 2, 1])).to.be(false);
        EXPECT_FALSE(isSorted(std::vector<ol::number_t>({ 1, 2, 1 })));
    }
    //    });
    //
    //    it('works with strict comparison without compare function', function() {
    {
        ...
        EXPECT_TRUE(isSorted(std::vector<ol::number_t>({ 1, 2, 3 }), 0, true));
        //      expect(isSorted([1, 2, 2], null, true)).to.be(false);
        EXPECT_FALSE(isSorted(std::vector<ol::number_t>({ 1, 2, 2 }), 0, true));
        //      expect(isSorted([1, 2, 1], null, true)).to.be(false);
        EXPECT_FALSE(isSorted(std::vector<ol::number_t>({ 1, 2, 1 }), 0, true));
    }
    //    });
    //
    //    it('works with a compare function', function() {
    {
        ...
        //        return b - a;
        //      }
        auto compare = [](auto a, auto b) { return b - a; };
        //      expect(isSorted([1, 2, 3], compare)).to.be(false);
        EXPECT_FALSE(isSorted(std::vector<ol::number_t>({ 1, 2, 3 }), compare));
        //      expect(isSorted([3, 2, 2], compare)).to.be(true);
        EXPECT_TRUE(isSorted(std::vector<ol::number_t>({ 3, 2, 2 }), compare));
        //    });
        //  });
    }
}
